...
class gen_inv(models.TransientModel):
    ...
    def make_invoice(self, values):
        ...
        if invoice_search:
            ...
            if self._context.get('default_journal_id', False):
                journal = self.env['account.journal'].browse(values.get('journal_id') or self._context.get('default_journal_id'))
            ...
            inv_id = invoice_obj.create({
                ...
                'invoice_scope' : values.get('invoice_scope').lower(),
    ...
    def import_csv(self):
        ...
        if self.import_option == 'csv':
            if self.account_opt == 'default':
                keys = ['invoice', 'customer', 'currency', 'product', 'quantity', 'uom', 'description', 'price','discount','salesperson','tax','date', 'invoice_scope', 'journal_id']
            else:
                keys = ['invoice', 'customer', 'currency', 'product','account', 'quantity', 'uom', 'description', 'price','discount','salesperson','tax','date', 'invoice_scope', 'journal_id']
            ...
            for i in range(len(file_reader)):
                ...
                if values:
                    if i == 0:
                        ...
                        res.invoice_scope = values.get('invoice_scope').lower()
                        res.client_name = res.invoice_scope == 'i' and res.partner_id.name
